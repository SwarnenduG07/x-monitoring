...
import { createLogger, type Logger } from "@believe-x/shared";
...
export function setupBotCommands(bot: Telegraf, logger: Logger) {
	...
	bot.command("register", async (ctx) => {
		try {
			...
			try {
				...
				await ctx.reply(
					"Database connection error. Please make sure your database is set up correctly.",
				);
				return;
			}
			...
			if (!tokenInfo) {
				await ctx.reply(
					"❌ Invalid or unknown Solana token address. Please check the address and try again.",
				);
				return;
			}
			...
			if (!tokenInfo.market) {
				await ctx.reply(
					`⚠️ Warning: Token ${tokenInfo.symbol} doesn't appear to be actively traded on major DEXes. Continuing anyway...`,
				);
			}

			await ctx.reply(
				`✅ Token verified: ${tokenInfo.symbol} (${tokenInfo.name || tokenInfo.symbol})\nProcessing your request to monitor @${xHandle}...`,
			);
			...
			try {
				...
				try {
					...
					logger.error(
						"Database error when finding/creating account:",
						dbError,
					);
					await ctx.reply(
						"Error creating account in the database. Please check your database connection.",
					);
					return;
				}
				...
				try {
					...
					await ctx.reply(
						"Error creating token in the database. Please check your database connection.",
					);
					return;
				}
				...
				try {
					...
					if (existingSubscription) {
						if (!existingSubscription.active) {
							...
							await ctx.reply(
								`✅ Successfully reactivated your subscription to monitor @${xHandle} for ${tokenInfo.symbol} tokens!`,
							);
						} else {
							await ctx.reply(
								`You are already monitoring @${xHandle} for ${tokenInfo.symbol} tokens.`,
							);
						}
						return;
					}
					...
					await ctx.reply(
						`✅ Successfully registered to monitor @${xHandle} for ${tokenInfo.symbol} tokens!`,
					);
					logger.info(
						`User ${user.id} registered to monitor @${xHandle} for ${tokenInfo.symbol} (${tokenInfo.address})`,
					);
				} catch (dbError) {
					logger.error("Database error when creating subscription:", dbError);
					await ctx.reply(
						"Error creating subscription in the database. Please check your database connection.",
					);
				}
			} catch (error) {
				logger.error("Error registering subscription:", error);
				await ctx.reply(
					"Sorry, I could not register your subscription. Please try again later.",
				);
			}
		} catch (error) {
	...
	bot.command("unregister", async (ctx) => {
		try {
			...
			if (!subscription) {
				await ctx.reply(
					`You are not monitoring @${xHandle} for ${token.symbol} tokens.`,
				);
				return;
			}
			...
			await ctx.reply(
				`✅ Successfully unregistered from monitoring @${xHandle} for ${token.symbol} tokens.`,
			);
			logger.info(
				`User ${user.id} unregistered from monitoring @${xHandle} for ${token.symbol}`,
			);
		} catch (error) {
	...
	bot.command("lookup", async (ctx) => {
		try {
			...
			await ctx.reply(
				`Looking up token with symbol ${symbol.toUpperCase()}...`,
			);
			...
			try {
				...
				if (jupiterResponse.data && Array.isArray(jupiterResponse.data)) {
					const tokens = jupiterResponse.data.filter(
				...
				await ctx.reply(
					"Sorry, I could not look up the token. Please try again later.",
				);
			}
		} catch (error) {
