 ...
 */
export async function verifySolanaToken(address: string): Promise<SolanaTokenInfo | null> {
  try {
 ...
 */
async function tryMultipleAPIs(address: string): Promise<SolanaTokenInfo | null> {
  ...
  try {
    ...
    if (jupiterResponse.data && Array.isArray(jupiterResponse.data)) {
      const token = jupiterResponse.data.find((t: any) => t.address === address);
  ...
  try {
    ...
    if (dexscreenerResponse.data && dexscreenerResponse.data.pairs && dexscreenerResponse.data.pairs.length > 0) {
      ...
      const tokenData = pair.baseToken.address.toLowerCase() === address.toLowerCase() ? 
  ...
  try {
    ...
    const birdeyeResponse = await axios.get(`https://public-api.birdeye.so/public/tokenlist?blockchain=solana`, {
      timeout: 10000
    });
    
    if (birdeyeResponse.data && birdeyeResponse.data.data && Array.isArray(birdeyeResponse.data.data)) {
      const token = birdeyeResponse.data.data.find((t: any) => t.address === address);
...
async function checkTokenMarket(address: string): Promise<boolean> {
  try {
    ...
    return !!(response.data && response.data.pairs && response.data.pairs.length > 0);
  } catch (error: any) {
 ...
 */
export async function getTokenBySymbol(symbol: string): Promise<SolanaTokenInfo | null> {
  try {
    ...
    if (jupiterResponse.data && Array.isArray(jupiterResponse.data)) {
      const token = jupiterResponse.data.find((t: any) => 
