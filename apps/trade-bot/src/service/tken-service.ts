 ...
 */
export async function verifyTokenAddress(
	address: string,
	...
	try {
		...
		if (existingToken) {
			logger.info(
				`Token found in database: ${existingToken.symbol} (${existingToken.address})`,
			);
		...
		logger.info(
			`New token verified and saved: ${token.symbol} (${token.address})`,
		);
		return token;
	} catch (error) {
