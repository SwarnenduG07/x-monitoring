...
`;
				...
				if (analysis.decision === "buy" && analysis.confidence >= 0.8) {
					logger.info(
						`High confidence buy signal for ${tokenInfo.symbol}, initiating trade`,
					);

					// Call trading orchestrator directly (in a real system)
					// For now, just log that we would initiate a trade
					logger.info(
						`Would initiate trade for ${tokenInfo.symbol} based on analysis`,
					);
				}
		...
		try {
			...
			const tokenSubscriptions = new Map<
				number,
				{ token: any; subscriptions: typeof subscriptions }
			>();
...
`;
					...
					for (const subscription of subs) {
						try {
							...
							logger.info(
								`Analysis sent to user ${subscription.user.telegramId} for ${token.symbol}`,
							);
						} catch (error) {
							logger.error(
								`Failed to send message to user ${subscription.user.telegramId}:`,
								error,
							);
						}
					}
					...
					if (analysis.decision === "buy" && analysis.confidence >= 0.8) {
						logger.info(
							`High confidence buy signal for ${token.symbol}, initiating trade`,
						);

						// Call trading orchestrator directly (in a real system)
						// For now, just log that we would initiate a trade
						logger.info(
							`Would initiate trade for ${token.symbol} based on analysis`,
						);
					}
				} catch (error) {
