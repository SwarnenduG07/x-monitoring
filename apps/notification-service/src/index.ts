...
function formatAnalysisResultMessage(
	notification: NotificationMessage,
): string {
	...
	const positiveReasons = reasons.positiveSignals
		.map((r: string) => `✅ ${r}`)
		.join("\n");
	const negativeReasons = reasons.negativeSignals
		.map((r: string) => `❌ ${r}`)
		.join("\n");
	...
	if (analysis.marketConditions) {
		...
		if (marketConditions.relatedTokens?.length) {
			...
			marketConditions.relatedTokens.forEach(
				(token: { symbol: string; sentiment: number }) => {
					const sentiment =
...
function formatTradeExecutionMessage(
	notification: NotificationMessage,
): string {
...
async function initializeBot() {
	try {
		...
		bot.command("start", async (ctx: any) => {
			await ctx.reply(
				"👋 Welcome to the Believe X AI Trading Bot! I will send you notifications about new posts, analysis results, and trading activities.",
			);
		...
		bot.command("status", async (ctx: any) => {
			await ctx.reply(
				"⚙️ Systems operational. Monitoring X accounts and processing trading signals.",
			);
		...
		bot.command("help", async (ctx: any) => {
			await ctx.reply(
				"🔍 I monitor X accounts for new posts, analyze them with AI, and execute trades when signals are strong. You will receive notifications about each step of the process.",
			);
...
app.listen(PORT, () => {
	...
	const startupNotification: NotificationMessage = {
		...
		message:
			"The Believe X AI Trading Bot has been started and is now monitoring X accounts.",
		timestamp: new Date(),
	};
	sendNotification(startupNotification).catch((error) => {
