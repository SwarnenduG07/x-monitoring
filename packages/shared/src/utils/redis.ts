...
import { RedisMessage, RedisTopic } from '../types';
        ...
        if (handlers) {
          handlers.forEach(handler => {
            try {
              ...
              logger.error(`Error in Redis handler for channel ${channel}:`, err);
            }
          });
        }
      } catch (err) {
...
export const createRedisService = (redisUrl: string = process.env.REDIS_URL || 'redis://localhost:6379'): RedisService => {
